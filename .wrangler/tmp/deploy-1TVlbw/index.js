...
var getSupabaseClient = /* @__PURE__ */ __name((env) => {
  const supabaseUrl = env.SUPABASE_URL;
  const supabaseKey = env.SUPABASE_ANON_KEY;
  const serviceKey = env.SUPABASE_SERVICE_KEY;
  return {
    auth: {
      signUp: /* @__PURE__ */ __name(async (email, password, metadata) => {
      ...
      signIn: /* @__PURE__ */ __name(async (email, password) => {
      ...
      updateUserMetadata: /* @__PURE__ */ __name(async (userId, metadata, key = serviceKey) => {
    ...
    database: {
      update: /* @__PURE__ */ __name(async (table, data, filters) => {
      ...
      query: /* @__PURE__ */ __name(async (table, filters = {}) => {
        const query = new URLSearchParams();
        Object.entries(filters).forEach(([key, value]) => {
          query.append(`${key}=eq.${value}`, "");
        });
...
app.post("/login", async (c) => {
  try {
    ...
    return c.json({
      ...
      token: signInRes.access_token || signInRes?.session?.access_token,
      user: signInRes.user || signInRes?.user,
      session: {
        access_token: signInRes.access_token || signInRes?.session?.access_token,
        refresh_token: signInRes.refresh_token || signInRes?.session?.refresh_token,
        expires_in: signInRes.expires_in || signInRes?.session?.expires_in,
        token_type: signInRes.token_type || signInRes?.session?.token_type
      }
    });
  } catch (err) {
...
// src/storage/kv.js
var saveLstsRegistration = /* @__PURE__ */ __name(async (kv, registration) => {
...
var getLstsRegistration = /* @__PURE__ */ __name(async (kv, id) => {
  const data = await kv.get(`lsts:${id}`);
  return data ? JSON.parse(data) : null;
}, "getLstsRegistration");
var getUserLstsRegistrations = /* @__PURE__ */ __name(async (kv, userId) => {
...
var getWeeklyLstsRegistrations = /* @__PURE__ */ __name(async (kv) => {
  const weekKey = getWeekKey((/* @__PURE__ */ new Date()).toISOString());
  return getRegistrationsByWeek(kv, "lsts", weekKey);
}, "getWeeklyLstsRegistrations");
var getAllLstsRegistrations = /* @__PURE__ */ __name(async (kv) => {
...
var savePrayerRequest = /* @__PURE__ */ __name(async (kv, prayer) => {
  const id = prayer.id;
  await kv.put(
    `prayer:${id}`,
...
var getPrayerRequest = /* @__PURE__ */ __name(async (kv, id) => {
  const data = await kv.get(`prayer:${id}`);
  return data ? JSON.parse(data) : null;
}, "getPrayerRequest");
var getAllPrayers = /* @__PURE__ */ __name(async (kv) => {
...
var saveSummitRegistration = /* @__PURE__ */ __name(async (kv, registration) => {
  const id = registration.id;
  await kv.put(
    `summit:${id}`,
    JSON.stringify(registration),
...
var getSummitRegistration = /* @__PURE__ */ __name(async (kv, id) => {
  const data = await kv.get(`summit:${id}`);
  return data ? JSON.parse(data) : null;
}, "getSummitRegistration");
var getUserSummitRegistrations = /* @__PURE__ */ __name(async (kv, userId) => {
...
var getAllSummitRegistrations = /* @__PURE__ */ __name(async (kv) => {
...
var getWeekKey = /* @__PURE__ */ __name((dateString) => {
  const date = new Date(dateString);
  const year2 = date.getFullYear();
  const month = String(date.getMonth() + 1).padStart(2, "0");
  const week2 = Math.ceil(date.getDate() / 7);
  return `${year2}-${month}-w${week2}`;
}, "getWeekKey");
var addToWeekIndex = /* @__PURE__ */ __name(async (kv, type, weekKey, id) => {
...
var addToUserIndex = /* @__PURE__ */ __name(async (kv, userKey, id) => {
...
var addToGlobalIndex = /* @__PURE__ */ __name(async (kv, type, id) => {
...
var getRegistrationsByWeek = /* @__PURE__ */ __name(async (kv, type, weekKey) => {
  const indexKey = `index:${type}:${weekKey}`;
  const data = await kv.get(indexKey);
  if (!data) return [];
  const ids = JSON.parse(data);
...
// src/routes/lsts.js
var app2 = new Hono2();
var getWeekRange = /* @__PURE__ */ __name((date = /* @__PURE__ */ new Date()) => {
  const d = new Date(date);
  d.setHours(0, 0, 0, 0);
  const day2 = d.getDay();
  const diffToMonday = (day2 + 6) % 7;
...
app2.post("/", authMiddleware, async (c) => {
  try {
    const body = await c.req.json();
    const userId = c.get("userId");
    const userEmail = c.get("userEmail");
...
app2.get("/user/week", authMiddleware, async (c) => {
  try {
    const userId = c.get("userId");
...
app2.get("/user/all", authMiddleware, async (c) => {
  try {
    const userId = c.get("userId");
    const registrations = await getUserLstsRegistrations(c.env.LSTS_KV, userId);
    return c.json({
...
app2.get("/weekly", authMiddleware, async (c) => {
  try {
    ...
    const year2 = monday.getFullYear();
    const registrations = await getWeeklyLstsRegistrations(c.env.LSTS_KV);
    return c.json({
      weekLabel: `LSTS registrations for the ${weekNum} week of ${monthName} ${year2}`,
      registrations,
      count: registrations.length,
      weekRange: {
        start: monday.toISOString().split("T")[0],
        end: new Date(monday.getTime() + 4 * 24 * 60 * 60 * 1e3).toISOString().split("T")[0]
      }
    });
  } catch (err) {
...
app2.get("/", authMiddleware, async (c) => {
  try {
    const adminEmails = (c.env.ADMIN_EMAILS || "").split(",");
    if (!adminEmails.includes(c.get("userEmail"))) {
      return c.json({ error: "Admin access required" }, 403);
...
app2.get("/:id", authMiddleware, async (c) => {
  try {
...
var lsts_default = app2;

// src/routes/prayers.js
var app3 = new Hono2();
...
app3.get("/", authMiddleware, async (c) => {
  try {
    const adminEmails = (c.env.ADMIN_EMAILS || "").split(",");
    if (!adminEmails.includes(c.get("userEmail"))) {
      return c.json({ error: "Admin access required" }, 403);
...
var prayers_default = app3;

// src/routes/summit.js
var app4 = new Hono2();
app4.post("/", authMiddleware, async (c) => {
  try {
    const body = await c.req.json();
    const userId = c.get("userId");
    const registration = {
...
app4.get("/user/all", authMiddleware, async (c) => {
  try {
    const userId = c.get("userId");
...
app4.get("/", authMiddleware, async (c) => {
  try {
    const adminEmails = (c.env.ADMIN_EMAILS || "").split(",");
    if (!adminEmails.includes(c.get("userEmail"))) {
      return c.json({ error: "Admin access required" }, 403);
...
var summit_default = app4;

// src/routes/messages.js
var app5 = new Hono2();
app5.post("/upload", authMiddleware, async (c) => {
  try {
    const body = await c.req.formData();
    ...
    const date = body.get("date") || (/* @__PURE__ */ new Date()).toISOString().split("T")[0];
    if (!file) {
      return c.json({ error: "No file provided" }, 400);
    }
    const buffer = await file.arrayBuffer();
    const filename = `${category}/${date}-${v4_default()}-${file.name}`;
...
app5.get("/", authMiddleware, async (c) => {
  try {
    const adminEmails = (c.env.ADMIN_EMAILS || "").split(",");
    if (!adminEmails.includes(c.get("userEmail"))) {
      return c.json({ error: "Admin access required" }, 403);
...
app5.delete("/:filename", authMiddleware, async (c) => {
  try {
    const adminEmails = (c.env.ADMIN_EMAILS || "").split(",");
    if (!adminEmails.includes(c.get("userEmail"))) {
      return c.json({ error: "Admin access required" }, 403);
...
var messages_default = app5;

// src/routes/admin.js
var app6 = new Hono2();
app6.get("/check", authMiddleware, async (c) => {
  try {
    const userEmail = c.get("userEmail");
...
app6.post("/assign", authMiddleware, async (c) => {
  try {
    const userEmail = c.get("userEmail");
    const adminEmails = (c.env.ADMIN_EMAILS || "").split(",").map((e) => e.trim());
...
app6.get("/dashboard", authMiddleware, async (c) => {
  try {
    const adminEmails = (c.env.ADMIN_EMAILS || "").split(",");
    if (!adminEmails.includes(c.get("userEmail"))) {
      return c.json({ error: "Admin access required" }, 403);
...
app7.route("/api/lsts", lsts_default);
app7.route("/api/prayers", prayers_default);
app7.route("/api/summit", summit_default);
app7.route("/api/messages", messages_default);
